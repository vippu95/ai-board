...
const prompt = ai.definePrompt({
  ...
  prompt: `You are a moderator for a board meeting. Your task is to determine the roles of AI agents required for an effective discussion on the following topic: {{{topic}}}. Please provide a list of roles that would contribute to a balanced and thorough discussion. The roles should be distinct and cover different aspects of the topic. You shuld only select the most important participants, restrict yourself to a maxium of 6 board members. Return the list of roles.`,
});
